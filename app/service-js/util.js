...
const xlsx_data_1 = require("./xlsx-data");
const dialog = require("electron").dialog;
class Util {
    // 获取单元格解析内容中的实际值，v
    static get_cell_v(cell, fileName) {
        if (cell == null) {
            return "";
        }
        let val = cell.v;
    ...
    static get_cell_c(cell) {
        ...
        if (cell.c && cell.c.length > 0 && cell.c[0].h) {
            ...
            let retDict = Object.create(null);
    ...
    */
    static show_error_dialog = function (msg) {
        console.log(`\n 错误日志：${msg}`);
        if (!xlsx_data_1.default.instance.can_add_errorMsg()) {
            return;
        }
        if (!msg) {
            msg = "未知错误，请联系开发";
        ...
        xlsx_data_1.default.instance.set_export_state_str(msg);
        if (!Util.is_jenkins()) {
